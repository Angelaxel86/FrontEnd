...
window.addEventListener('DOMContentLoaded', () =>{
	...
	hrx.open('GET', 'https://swapi.dev/api/people');
	hrx.send();
	hrx.onreadystatechange = () =>{
		if(hrx.readyState === 4 && hrx.status === 200) {
			...
			show(JSON.parse(hrx.responseText).results);
...
function show (data) {
	data.forEach( (el)=>{
		...
		homeworld.open('GET', el.homeworld);
		homeworld.send();
		homeworld.onreadystatechange = () =>{
			if(homeworld.readyState === 4 && homeworld.status === 200) {
				//console.log( JSON.parse(homeworld.responseText) );
				let objHomeworld = JSON.parse(homeworld.responseText);
		...
		el.films.forEach( (e) => {
			...
			films.open('GET', e);
			films.send();
			films.onreadystatechange = () =>{
				if(films.readyState === 4 && films.status === 200) {
					let objFilms = JSON.parse(films.responseText);
...
window.addEventListener("DOMContentLoaded",()=>{
	...
	btn.addEventListener('click', (e) => {		
		...
		const hrx = new XMLHttpRequest();
		hrx.open('GET', `https://swapi.dev/api/people/?page=${key}`);
		hrx.send();
		hrx.onreadystatechange = () =>{
			if(hrx.readyState === 4 && hrx.status === 200) {			
				show(JSON.parse(hrx.responseText).results);
